...
export default function QuestionBank() {
	...
	useEffect(() => {
		...
		if (isMounted) {
			if (editingItemID) {
				axiosInstance({ method: 'get', url: `/questions/auth/${editingItemID}` })
					.then(function (response) {
	...
	const handleAdd = () => {
		if (newItemTitle && newItemDriveId && newItemClass && newItemDepartment && newItemSubject) {
			const config = {
				method: 'post',
				url: `/questions/auth`,
				data: {
					title: newItemTitle,
					driveId: newItemDriveId,
					class: newItemClass,
	...
	const handleSave = () => {
		if (editingItemID) {
			const config = {
				method: 'patch',
				url: `/questions/auth/${editingItemID}`,
				data: {
					title: newItemTitle ? newItemTitle : editingItem.title,
					driveId: newItemDriveId ? newItemDriveId : editingItem.driveId,
